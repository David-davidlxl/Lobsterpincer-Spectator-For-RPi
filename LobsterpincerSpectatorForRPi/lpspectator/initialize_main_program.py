"""This module is responsible for initializing the main program.

It is invoked at the beginning of the execution of the main program
...
from lpspectator.configure_led import (
    set_up_gpio_for_led,
    turn_on_led_lights,
)
from lpspectator.configure_lcd import set_up_lcd
...
def initialize_lpspectator(
    ...
    :return: All variables relevant to execution of the main program.
    """
    ...
    try:
        ...
        print(
            "\tFailed to initialize the Stockfish engine because of the"
            " lack of permission"
        )
        print(
    ...
    if board.is_checkmate() or board.is_stalemate():
        print(
            "\tFailed to initialize the board because there's no legal "
            "move in the starting position"
        )
        print(
            "\t\tPlease edit the `FULL_FEN_OF_STARTING_POSITION` "
    ...
    if previous_img is None:
        ...
        print(
            "\t\tPlease edit the `IMAGE_SOURCE` variable and rerun the program"
        )
        quit_engine(engine)
        sys.exit()
    ...
    if board_corners is None:
        print(
            "Now make the first move and tune the slider values in the "
            "trackbar window until the perspective-transformed image contains "
            "a clear view of the chessboard"
        )
    else:
        # `board_corners` is set equal to
        # `[[0, 0], [1199, 0], [1199, 1199], [0, 1199]]`
        print(
            "Now make the first move and tune the slider values in the "
            "trackbar window until the perspective-transformed image contains "
            "precisely the 64 squares of the chessboard"
        )
